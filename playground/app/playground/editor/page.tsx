...
const CUSTOM_SOL = `// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

/// Your own contract — write or paste Solidity here.
/// The seal0 Rust on the right regenerates as you type, then
/// compile → deploy → call against the live Portaldot node.
contract MyContract {
...
function EditorInner() {
  ...
  const isCustom = !contractParam || contractParam === 'custom';

  const [solidity, setSolidity] = useState(isCustom ? CUSTOM_SOL : '');
  const [loadingSource, setLoadingSource] = useState(!isCustom);
  ...
  /* Load the chosen contract's ACTUAL local source (so it matches what compiles). */
  useEffect(() => {
    if (isCustom) { setSolidity(CUSTOM_SOL); setLoadingSource(false); return; }
    let cancelled = false;
    setLoadingSource(true);
    fetch(`/api/source?contract=${encodeURIComponent(contractParam!)}`)
      .then(async r => {
        const data = await r.json() as { source?: string; error?: string };
        if (cancelled) return;
        if (r.ok && data.source) {
          setSolidity(data.source);
        } else {
          // Fall back to a minimal starter so the editor is never empty.
          setSolidity(`// Could not load "${contractParam}" — starter contract loaded instead.\n${CUSTOM_SOL}`);
        }
        setLoadingSource(false);
      })
      .catch(() => {
        if (cancelled) return;
        setSolidity(CUSTOM_SOL);
        setLoadingSource(false);
      });
    return () => { cancelled = true; };
  }, [contractParam, isCustom]);
  ...
  return (
    <div className="app">
      ...
      <div className="topbar">
        <a href="/playground" className="topbar-back">
...
export default function EditorPage() {
  return (
    <Suspense fallback={<div className="app"><div className="pg-vignette" /></div>}>
      <EditorInner />
    </Suspense>
  );
}
